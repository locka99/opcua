...
// NOTE: serde is DERIVED (not the base64/human-readable impl) so the type round-trips through
// postcard, which the fuzzer uses for corpus/state (de)serialization. The old manual impl always
// serialized as a base64 string with no `is_human_readable` guard, so postcard restore hit
// `Error::custom("Cannot decode base64 bytestring")` (the SerdeDeCustom restart-loop). TLS/SSH use
// derived serde for the same reason. base64 helpers (`as_base64`/`from_base64`) are kept for callers.
#[derive(Eq, PartialEq, Debug, Clone, Hash, Extractable, Serialize, Deserialize)]
