...
use crate::{
    ...
    service_types::StatusResult, string::UAString, variant::Variant,
};
...
impl BinaryEncoder<ProgramDiagnostic2DataType> for ProgramDiagnostic2DataType {
    ...
    fn decode<S: Read>(
        ...
        let last_method_output_arguments: Option<Vec<Argument>> =
            read_array(stream, decoding_limits)?;
        let last_method_input_values: Option<Vec<Variant>> =
            read_array(stream, decoding_limits)?;
        let last_method_output_values: Option<Vec<Variant>> =
            read_array(stream, decoding_limits)?;
