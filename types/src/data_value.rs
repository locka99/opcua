...
use crate::{
    byte_string::ByteString, date_time::*, encoding::*, guid::Guid,
    localized_text::LocalizedText, node_id::NodeId, qualified_name::QualifiedName,
    service_types::TimestampsToReturn, status_codes::StatusCode, string::UAString,
    variant::Variant,
};
...
impl BinaryEncoder<DataValue> for DataValue {
    ...
    fn decode<S: Read>(
        ...
        let status = if encoding_mask.contains(DataValueFlags::HAS_STATUS) {
            let status =
                StatusCode::from_bits_truncate(u32::decode(stream, decoding_limits)?);
            Some(status)
        } else {
            None
        };
