...
use crate::{
    ...
    service_types::{
        ...
        MonitoredItemNotification, NotificationMessage, StatusChangeNotification,
    },
        ...
        if let Some(ref notification_data) = self.notification_data {
            ...
            notification_data.iter().for_each(|n| {
                if n.node_id == data_change_notification_id {
                    if let Ok(v) =
                        n.decode_inner::<DataChangeNotification>(decoding_limits)
                    {
                        data_changes.push(v);
                    }
                } else if n.node_id == event_notification_list_id {
                    if let Ok(v) =
                        n.decode_inner::<EventNotificationList>(decoding_limits)
                    {
                        events.push(v);
                    }
                } else {
