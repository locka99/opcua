...
use crate::{
    ...
    ByteString, DataTypeId, DateTime, ExpandedNodeId, Guid, LocalizedText, NodeId,
...
fn variant_type_id() {
    ...
    use crate::{
        ByteString, DateTime, ExpandedNodeId, ExtensionObject, Guid, LocalizedText,
        NodeId, QualifiedName, UAString, XmlElement,
    };
...
fn variant_multi_dimensional_array() {
    ...
    let v =
        Variant::from((vec![Variant::from(10), Variant::from(10)], vec![1u32, 2u32]));
    assert!(v.is_array());
    assert!(v.is_array_of_type(VariantTypeId::Int32));
    assert!(v.is_valid());
...
fn variant_convert_string() {
    ...
    // Guid
    assert_eq!(
        Variant::from("d47a32c9-5ee7-43c1-a733-0fe30bf26b50")
            .convert(VariantTypeId::Guid),
