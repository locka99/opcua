    ...
    fn try_from(value: &NodeId) -> Result<Self, Self::Error> {
        if value.namespace == 0 {
            if let Identifier::Numeric(type_id) = value.identifier {
                match type_id {
                    type_id if type_id == DataTypeId::Boolean as u32 => {
                        Ok(VariantTypeId::Boolean)
                    }
                    type_id if type_id == DataTypeId::Byte as u32 => {
                        Ok(VariantTypeId::Byte)
                    }
                    type_id if type_id == DataTypeId::Int16 as u32 => {
                        Ok(VariantTypeId::Int16)
                    }
                    type_id if type_id == DataTypeId::UInt16 as u32 => {
                        Ok(VariantTypeId::UInt16)
                    }
                    type_id if type_id == DataTypeId::Int32 as u32 => {
                        Ok(VariantTypeId::Int32)
                    }
                    type_id if type_id == DataTypeId::UInt32 as u32 => {
                        Ok(VariantTypeId::UInt32)
                    }
                    type_id if type_id == DataTypeId::Int64 as u32 => {
                        Ok(VariantTypeId::Int64)
                    }
                    type_id if type_id == DataTypeId::UInt64 as u32 => {
                        Ok(VariantTypeId::UInt64)
                    }
                    type_id if type_id == DataTypeId::Float as u32 => {
                        Ok(VariantTypeId::Float)
                    }
                    type_id if type_id == DataTypeId::Double as u32 => {
                        Ok(VariantTypeId::Double)
                    }
                    type_id if type_id == DataTypeId::String as u32 => {
                        Ok(VariantTypeId::String)
                    }
                    type_id if type_id == DataTypeId::DateTime as u32 => {
                        Ok(VariantTypeId::DateTime)
                    }
                    type_id if type_id == DataTypeId::Guid as u32 => {
                        Ok(VariantTypeId::Guid)
                    }
                    ...
                    type_id if type_id == DataTypeId::NodeId as u32 => {
                        Ok(VariantTypeId::NodeId)
                    }
    ...
    ($rtype: ident) => {
        ...
        impl<'a> From<&'a [$rtype]> for Variant {
            fn from(v: &'a [$rtype]) -> Self {
                let array: Vec<Variant> =
                    v.iter().map(|v| Variant::from(v.clone())).collect();
                Variant::from(array)
            }
        }
    };
...
impl BinaryEncoder<Variant> for Variant {
    ...
    fn decode<S: Read>(
        ...
        let element_encoding_mask =
            encoding_mask & !(ARRAY_DIMENSIONS_BIT | ARRAY_VALUES_BIT);
...
impl Variant {
    ...
    fn encode_variant_value<S: Write>(
        stream: &mut S,
        value: &Variant,
            ...
            match *self {
                ...
                Variant::Guid(ref v) => match target_type {
                    ...
                    VariantTypeId::ByteString => {
                        ByteString::from(v.as_ref().clone()).into()
                    }
                    _ => Variant::Empty,
                },
                ...
                Variant::Int32(v) => match target_type {
                    ...
                    VariantTypeId::StatusCode => {
                        (StatusCode::from_bits_truncate(v as u32)).into()
                    }
                    VariantTypeId::String => format!("{}", v).into(),
                ...
                Variant::StatusCode(v) => match target_type {
                    VariantTypeId::UInt16 => {
                        (((v.bits() & 0xffff_0000) >> 16) as u16).into()
                    }
                    _ => Variant::Empty,
                },
                ...
                Variant::UInt32(v) => match target_type {
                    ...
                    VariantTypeId::StatusCode => {
                        StatusCode::from_bits_truncate(v).into()
                    }
                    VariantTypeId::String => format!("{}", v).into(),
                ...
                Variant::UInt64(v) => match target_type {
                    ...
                    VariantTypeId::StatusCode => StatusCode::from_bits_truncate(
                        (v & 0x0000_0000_ffff_ffff) as u32,
                    )
                    .into(),
                    VariantTypeId::String => format!("{}", v).into(),
    ...
    pub fn convert(&self, target_type: VariantTypeId) -> Variant {
        ...
        match *self {
            ...
            Variant::NodeId(ref v) => {
                ...
                match target_type {
                    VariantTypeId::ExpandedNodeId => {
                        ExpandedNodeId::from(*v.clone()).into()
                    }
                    VariantTypeId::String => format!("{}", v).into(),
                    _ => Variant::Empty,
                }
            ...
            Variant::ByteString(values) => match &values.value {
                ...
                Some(values) => {
                    let values: Vec<Variant> =
                        values.iter().map(|v| Variant::Byte(*v)).collect();
                    Variant::from(Array::new_single(values))
                }
            },
            _ => panic!(),
                    ...
                    Variant::Array(array) => {
                        ...
                        if min >= values.len() {
                            ...
                            let vals: Vec<Variant> =
                                vals.iter().map(|v| v.clone()).collect();
