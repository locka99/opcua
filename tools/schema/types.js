...
/// This code parses a OPC UA BSD file and generates a Rust module containing the types.
...
exports.from_xml = (bsd_file, rs_module, codegen_info) => {
    ...
    fs.readFile(bsd_file, (err, data) => {
        parser.parseString(data, (err, result) => {
            ...
            generate_types(data, rs_module, codegen_info);
        });
    });
}

function generate_types(data, rs_types_dir, codegen_info) {
    // Output module
    generate_types_mod(data.structured_types, rs_types_dir, codegen_info);

    // Output structured types
    _.each(data.structured_types, structured_type => {
        generate_structured_type_file(data.structured_types, structured_type, rs_types_dir, codegen_info);
    });

    // Output enums
    generate_enum_types(data.enums, rs_types_dir, codegen_info);
}

function generate_types_mod(structured_types, rs_types_dir, codegen_info) {
...
function generate_enum_types(enums, rs_types_dir, codegen_info) {
    let contents = `// OPCUA for Rust
// SPDX-License-Identifier: MPL-2.0
// Copyright (C) 2017-2022 Adam Lock
// ${codegen_info.autogen_comment}
...
use ${codegen_info.types_crate}::encoding::*;
use ${codegen_info.types_crate}::status_code::StatusCode;
`;

    let needs_bitflags = _.some(enums, e => e.option);
    if (needs_bitflags) {
        contents += `
use bitflags;
`;
...
function generate_type_imports(structured_types, fields_to_add, fields_to_hide, has_message_info, codegen_info) {
    let imports = `#[allow(unused_imports)]
`;

    if (codegen_info.custom_bsd) {
        imports += `use super::*;
use ${codegen_info.types_crate}::*;
`;
    } else {
        imports += `use ${codegen_info.types_crate}::{
    encoding::*,
    basic_types::*,
`;
...
function generate_structured_type_file(structured_types, structured_type, rs_types_dir, codegen_info) {
    let file_name = _.snakeCase(structured_type.name) + ".rs";
    let file_path = `${rs_types_dir}/${file_name}`;

    let has_message_info = _.has(structured_type, "base_type") && structured_type.base_type === "ua:ExtensionObject" && !codegen_info.custom_bsd;
...
use std::io::{Read, Write};
`;
    contents += generate_type_imports(structured_types, structured_type.fields_to_add, structured_type.fields_to_hide, has_message_info, codegen_info);
    contents += "\n";
