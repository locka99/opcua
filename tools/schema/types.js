...
exports.from_xml = (bsd_file, rs_module) => {
    ...
    fs.readFile(bsd_file, (err, data) => {
        parser.parseString(data, (err, result) => {
            ...
            generate_types(data, rs_module);
        });
    });
}

function generate_types(data, rs_types_dir) {
    // Output module
    generate_types_mod(data.structured_types, rs_types_dir);

    // Output structured types
    _.each(data.structured_types, structured_type => {
        generate_structured_type_file(data.structured_types, structured_type, rs_types_dir);
    });

    // Output enums
    generate_enum_types(data.enums, rs_types_dir);
}

function generate_types_mod(structured_types, rs_types_dir) {
...
function generate_enum_types(enums, rs_types_dir) {
    let contents = `// OPCUA for Rust
// SPDX-License-Identifier: MPL-2.0
// Copyright (C) 2017-2022 Adam Lock
// This file was autogenerated from Opc.Ua.Types.bsd.xml by tools/schema/gen_types.js
...
function generate_structured_type_file(structured_types, structured_type, rs_types_dir) {
