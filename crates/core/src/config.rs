...
pub trait Config: serde::Serialize {
        ...
        if self.is_valid() {
            ...
            if let Ok(mut f) = File::create(path) {
                ...
                if result.is_ok() {
                    ...
                    log::error!("Could not save config - error = {:?}", result.unwrap_err())
                }
            } else {
                log::error!("Cannot create the path to save the config");
            }
        } else {
            log::error!("Config isn't valid and won't be saved");
        }
        Err(())
    }
        ...
        for<'de> A: Config + serde::Deserialize<'de>,
    {
        if let Ok(mut f) = File::open(path) {
            ...
            log::error!("Cannot open configuration file {}", path.to_string_lossy());
            Err(())
        }
    }
