...
fn user_name_identity_token_valid() {
    ...
    assert!(!id.is_valid());
    id.user_name = UAString::from_str("x");
    assert!(!id.is_valid());
    id.user_name = UAString::null();
    id.password = ByteString::from_bytes(b"xyz");
    assert!(!id.is_valid());
    id.user_name = UAString::from_str("x");
    assert!(id.is_valid());
}
...
fn user_name_identity_token_plaintext() {
    let mut id = UserNameIdentityToken {
        policy_id: UAString::null(),
        user_name: UAString::from_str("xyz"),
        password: ByteString::from_bytes(b"pwd1"),
        encryption_algorithm: UAString::null(),
    };
    ...
    id.password = ByteString::from_bytes(b"");
    let result = id.authenticate("xyz", b"");
    assert!(result.is_ok());

    id.user_name = UAString::from_str("");
