...
fn hello_data() -> Vec<u8> {
    vec![
        0x48, 0x45, 0x4c, 0x46, 0x39, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x0a, 0x00, 0x00, 0x00, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x19, 0x00, 0x00, 0x00, 0x6f, 0x70, 0x63, 0x2e, 0x74, 0x63, 0x70,
        0x3a, 0x2f, 0x2f, 0x31, 0x32, 0x37, 0x2e, 0x30, 0x2e, 0x30, 0x2e, 0x31, 0x3a,
        0x31, 0x32, 0x33, 0x34, 0x2f,
    ]
}

fn ack_data() -> Vec<u8> {
    vec![
        0x41, 0x43, 0x4b, 0x46, 0x1c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x08, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x01, 0xff, 0xff,
        0x00, 0x00,
    ]
}
...
pub fn hello() {
    ...
    let hello = HelloMessage::decode(&mut stream, &decoding_limits).unwrap();
    println!("hello = {:?}", hello);
...
pub fn acknowledge() {
    ...
    let decoding_limits = DecodingLimits::default();
    let ack = AcknowledgeMessage::decode(&mut stream, &decoding_limits).unwrap();
    println!("ack = {:?}", ack);
