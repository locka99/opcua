...
impl SecureChannel {
    ...
    pub fn make_security_header(
        &self,
        message_type: MessageChunkType,
    ) -> SecurityHeader {
        match message_type {
            MessageChunkType::OpenSecureChannel => {
                let asymmetric_security_header =
    ...
    pub fn create_random_nonce(&mut self) {
        ...
        {
            self.local_nonce =
                vec![0u8; self.security_policy.secure_channel_nonce_length()];
    ...
    pub fn token_has_expired(&self) -> bool {
        ...
        let token_created_at: chrono::DateTime<chrono::Utc> =
            self.token_created_at.clone().into();
        let token_expires =
    ...
    pub fn apply_security(
                ...
                || self.security_mode == MessageSecurityMode::SignAndEncrypt)
        {
            ...
            let encrypted_size =
                if message_chunk.is_open_secure_channel(&self.decoding_limits) {
                    ...
                    let signed_range = 0..(data.len()
                        - self.security_policy.symmetric_signature_size());
                    self.symmetric_sign_and_encrypt(
                        &data,
                        signed_range,
                        encrypted_range,
                        dst,
    ...
    pub fn verify_and_remove_security_forensic(
        ...
        let (message_header, security_header, encrypted_data_offset) = {
            ...
            let message_header =
                MessageChunkHeader::decode(&mut stream, &self.decoding_limits)?;
    ...
    fn verify_padding(
        ...
        let padding_range = if key_size > 256 {
            ...
            let padding_size =
                ((extra_padding_byte as usize) << 8) + (padding_byte as usize);
    ...
    fn asymmetric_decrypt_and_verify(
        ...
        if our_thumbprint.value() != receiver_thumbprint.as_ref() {
            ...
            let signature_range_dst = signature_dst_offset
                ..(signature_dst_offset + verification_key_signature_size);
            ...
            let padding_range = self.verify_padding(
                dst,
    ...
    pub fn symmetric_sign_and_encrypt(
        ...
        let encrypted_size = match self.security_mode {
            MessageSecurityMode::None => {
                trace!(
                    "encrypt_and_sign is doing nothing because security mode == None"
                );
            ...
            MessageSecurityMode::SignAndEncrypt => {
                ...
                dst[..encrypted_range.start]
                    .copy_from_slice(&dst_tmp[..encrypted_range.start]);
    ...
    pub fn symmetric_decrypt_and_verify(
        ...
        match self.security_mode {
            ...
            MessageSecurityMode::SignAndEncrypt => {
                ...
                dst[..encrypted_range.start]
                    .copy_from_slice(&src[..encrypted_range.start]);
