...
use opcua_types::{
    ...
    service_types::{
        ...
        MonitoredItemModifyRequest, MonitoredItemNotification, ReadValueId,
        TimestampsToReturn,
    },
...
impl MonitoredItem {
    ...
    pub fn validate_filter(
        ...
        let filter_result =
            if let FilterType::EventFilter(ref event_filter) = self.filter {
    ...
    pub fn tick(
        ...
        if self.monitoring_mode == MonitoringMode::Disabled {
            ...
            let value_changed = check_value && {
                ...
                let first_tick =
                    !self.is_event_filter() && self.last_data_value.is_none();
    ...
    fn check_for_events(
        ...
        match self.filter {
            FilterType::EventFilter(ref filter) => {
                ...
                if Self::get_event_notifier(node)
        ...
        T: Into<Notification>,
    {
        ...
        if overflow {
            if let Notification::MonitoredItemNotification(ref mut notification) =
                notification
            {
