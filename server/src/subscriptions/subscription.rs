...
use opcua_types::{
    service_types::{
        MonitoredItemCreateRequest, MonitoredItemCreateResult,
        MonitoredItemModifyRequest, MonitoredItemModifyResult, NotificationMessage,
        TimestampsToReturn,
    },
...
impl Subscription {
    pub fn new(
        ...
        let subscription = Subscription {
            ...
            publishing_interval,
            priority,
            monitored_items: HashMap::with_capacity(
                constants::DEFAULT_MONITORED_ITEM_CAPACITY,
            ),
    ...
    fn monitored_item_create_error(
        status_code: StatusCode,
                        ...
                        Ok(monitored_item) => {
                                    ...
                                    <= max_monitored_items_per_sub
                            {
                                let revised_sampling_interval =
                                    monitored_item.sampling_interval();
                                let revised_queue_size =
                                    monitored_item.queue_size() as u32;
                    ...
                    Some(monitored_item) => {
                        ...
                        match modify_result {
                            Ok(filter_result) => MonitoredItemModifyResult {
                                status_code: StatusCode::Good,
                                revised_sampling_interval: monitored_item
                                    .sampling_interval(),
    ...
    fn test_and_set_publishing_interval_elapsed(&mut self, now: &DateTimeUtc) -> bool {
        ...
        let elapsed =
            now.signed_duration_since(self.last_time_publishing_interval_elapsed);
    ...
    pub(crate) fn tick(
        ...
        let publishing_interval_elapsed = match tick_reason {
            ...
            TickReason::TickTimerFired => {
                if self.state == SubscriptionState::Creating {
                    ...
                    panic!(
                        "Publishing interval should have been revised to min interval"
                    )
                } else {
        ...
        let notifications_available =
            !self.notifications.is_empty() || notification.is_some();
    ...
    fn tick_monitored_items(
        ...
        let mut monitored_item_notifications =
            Vec::with_capacity(self.monitored_items.len() * 2);
                    ...
                    if publishing_interval_elapsed {
                            ...
                            monitored_item.all_notifications()
                        {
                            monitored_item_notifications
                                .append(&mut item_notification_messages);
                        }
                    }
                }
    ...
    fn validate_triggered_items(
        ...
        let results: Vec<StatusCode> =
            items.iter().map(is_good_monitored_item_result).collect();
    ...
    pub(crate) fn set_triggering(
            ...
            self.monitored_items.get_mut(&monitored_item_id)
        {
            ...
            monitored_item
                .set_triggering(items_to_add.as_slice(), items_to_remove.as_slice());
