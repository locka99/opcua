...
use opcua_types::{
    service_types::{
        NotificationMessage, PublishRequest, PublishResponse, ServiceFault,
    },
...
impl Subscriptions {
    ...
    #[cfg(test)]
    pub(crate) fn publish_request_queue(
        &mut self,
    ) -> &mut VecDeque<PublishRequestEntry> {
        &mut self.publish_request_queue
    }

    #[cfg(test)]
    pub(crate) fn publish_response_queue(
        &mut self,
    ) -> &mut VecDeque<PublishResponseEntry> {
    ...
    pub fn take_publish_responses(&mut self) -> Option<VecDeque<PublishResponseEntry>> {
        if self.publish_response_queue.is_empty() {
            ...
            let mut publish_responses =
                VecDeque::with_capacity(self.publish_response_queue.len());
    ...
    pub(crate) fn tick(
        ...
        for subscription_id in subscription_ids {
            ...
            loop {
                if !self.publish_request_queue.is_empty() {
                    ...
                    {
                        let publish_request =
                            self.publish_request_queue.pop_back().unwrap();
        ...
        while !self.transmission_queue.is_empty() {
            ...
            let available_sequence_numbers =
                self.available_sequence_numbers(subscription_id);
    ...
    fn process_subscription_acknowledgements(
        ...
        if let Some(ref subscription_acknowledgements) =
            request.subscription_acknowledgements
        {
    ...
    fn remove_old_unacknowledged_notifications(&mut self) {
        ...
        if self.retransmission_queue.len() > max_retransmission_queue {
            let remove_count =
                self.retransmission_queue.len() - max_retransmission_queue;
            let sequence_nrs_to_remove = self
                .retransmission_queue
                .iter()
                .take(remove_count)
