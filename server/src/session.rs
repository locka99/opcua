...
impl Session {
    pub fn new() -> Session {
        ...
        Session {
            ...
            browse_continuation_points: Vec::with_capacity(super::constants::MAX_BROWSE_CONTINUATION_POINTS),
    ...
    pub fn add_browse_continuation_point(&mut self, continuation_point: BrowseContinuationPoint) {
        ...
        while self.browse_continuation_points.len() > super::constants::MAX_BROWSE_CONTINUATION_POINTS {
            let _ = self.browse_continuation_points.remove(0);
        }
    }

    /// Find a continuation point by id
    pub fn find_browse_continuation_point(&self, id: &ByteString) -> Option<&BrowseContinuationPoint> {
        self.browse_continuation_points.iter().find(|continuation_point| {
            continuation_point.id.eq(id)
        })
    }

    pub fn release_browse_continuation_point(&mut self, continuation_point_id: &ByteString) {
    ...
    pub fn release_browse_continuation_points(&mut self, continuation_points: &[ByteString]) {
