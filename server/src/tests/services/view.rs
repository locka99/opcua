...
fn do_browse_next(
    ...
    let request =
        make_browse_next_request(continuation_point, release_continuation_points);
...
fn browse_next_no_cp1() {
    do_view_service_test(|server_state, session, address_space, vs| {
        let parent_node_id =
            add_many_vars_to_address_space(address_space.clone(), 100).0;
...
fn translate_browse_paths_to_node_ids() {
    do_view_service_test(|server_state, _session, address_space, vs| {
        ...
        let response =
            vs.translate_browse_paths_to_node_ids(server_state, address_space, &request);
