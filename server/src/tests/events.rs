...
use opcua_types::{
    ...
    AttributeId, DataTypeId, LocalizedText, NodeId, ObjectId, ObjectTypeId,
    QualifiedName, UAString, VariableTypeId, Variant,
};

use crate::{
    address_space::{
        object_type::ObjectTypeBuilder, variable::VariableBuilder, AddressSpace,
    },
...
fn test_where_clause() {
    ...
    expected.into_iter().for_each(
        |(node_id, browse_path, value_to_compare, expected)| {
            let f = ContentFilterBuilder::new()
                .is_eq(
                ...
                .build();
            let result =
                event_filter::evaluate_where_clause(&node_id, &f, &address_space);
