...
pub fn validate(
    ...
    let select_clause_results =
        if let Some(ref select_clauses) = event_filter.select_clauses {
            Some(
                select_clauses
                    .iter()
    ...
    let where_clause_result =
        validate_where_clause(&event_filter.where_clause, address_space)?;
...
pub fn evaluate(
    ...
    if let Some(events) = events_for_object(object_id, address_space, happened_since) {
        let event_fields = events
            .iter()
            .filter(|event_id| {
                if let Ok(result) = evaluate_where_clause(
                    event_id,
                    ...
                    if let Some(ref select_clauses) = event_filter.select_clauses {
                        Some(
                            select_clauses
                                .iter()
                                .map(|v| {
                                    operator::value_of_simple_attribute(
                                        event_id,
                                        v,
                                        address_space,
                                    )
...
fn validate_select_clause(
    ...
    if !clause.index_range.is_empty() {
        ...
        if let Ok(node) = find_node_from_browse_path(
            &address_space,
