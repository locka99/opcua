...
use opcua_types::{
    ...
    AttributeId, ExtensionObject, NodeId, NumericRange, QualifiedName,
    TimestampsToReturn, Variant, VariantTypeId,
};
...
fn make_filter_operands(
    filter_operands: &[ExtensionObject],
) -> Result<Vec<Operand>, StatusCode> {
...
pub(crate) fn value_of_simple_attribute(
    ...
    if let Some(ref browse_path) = o.browse_path {
        ...
        if let Ok(node) =
            find_node_from_browse_path(address_space, object_id, browse_path)
        {
            match node {
...
pub(crate) fn gte(
    ...
    Ok(
        (result == ComparisonResult::GreaterThan || result == ComparisonResult::Equals)
            .into(),
    )
}
...
pub(crate) fn lte(
    ...
    Ok(
        (result == ComparisonResult::LessThan || result == ComparisonResult::Equals)
            .into(),
    )
}
