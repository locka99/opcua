...
use opcua_types::{
    service_types::TimeZoneDataType, AttributeId, ByteString, DataTypeId, DateTime,
    DateTimeUtc, ExtensionObject, Guid, LocalizedText, NodeId, NumericRange, ObjectId,
    ObjectTypeId, QualifiedName, TimestampsToReturn, UAString, VariableTypeId, Variant,
};
                ...
                if let Some(event_time) = event_time(event_id, address_space) {
                    ...
                    if time_predicate(&event_time.as_chrono()) {
                        if let Some(source_node) =
                            event_source_node(event_id, address_space)
                        {
