...
use opcua_types::{
    ...
    service_types::{
        ...
        ServerState as ServerStateType, SignatureData, UserNameIdentityToken,
...
use crate::{
    ...
    identity_token::{
        IdentityToken, POLICY_ID_ANONYMOUS, POLICY_ID_USER_PASS_NONE,
        POLICY_ID_USER_PASS_RSA_15, POLICY_ID_USER_PASS_RSA_OAEP, POLICY_ID_X509,
    },
};
...
pub struct ServerState {
    ...
    pub(crate) historical_data_provider:
        Option<Box<dyn HistoricalDataProvider + Send + Sync>>,
    /// Callback for historical events
    pub(crate) historical_event_provider:
        Option<Box<dyn HistoricalEventProvider + Send + Sync>>,
}
...
impl ServerState {
    pub fn endpoints(
        ...
        if let Some(ref transport_profile_uris) = *transport_profile_uris {
            ...
            if !transport_profile_uris.is_empty() {
                ...
                let found_binary_transport =
                    transport_profile_uris.iter().any(|profile_uri| {
    ...
    fn new_endpoint_description(
        ...
        EndpointDescription {
            ...
            transport_profile_uri: UAString::from(
                profiles::TRANSPORT_PROFILE_URI_BINARY,
            ),
    ...
    pub fn authenticate_endpoint(
        ...
        if let Some(endpoint) =
            config.find_endpoint(endpoint_url, security_policy, security_mode)
        {
            ...
            match IdentityToken::new(user_identity_token, &self.decoding_limits()) {
                ...
                IdentityToken::X509IdentityToken(token) => self
                    .authenticate_x509_identity_token(
                        &config,
                        endpoint,
                        &token,
        ...
        if !endpoint.supports_x509(&config.user_tokens) {
            ...
            let result = match server_certificate {
                Some(ref server_certificate) => {
                    ...
                    let user_identity_tokens =
                        self.user_identity_tokens(config, endpoint);
            ...
            result.and_then(|_| {
                ...
                for user_token_id in &endpoint.user_token_ids {
                    if let Some(server_user_token) =
                        config.user_tokens.get(user_token_id)
                    {
