...
impl References {
        ...
        references.iter().for_each(|r| {
            ...
            match r.2 {
                ReferenceDirection::Forward => {
                    self.insert_reference(source_node, r.0, r.1)
                }
                ReferenceDirection::Inverse => {
                    self.insert_reference(r.0, source_node, r.1)
                }
            };
        });
    }
    ...
    fn remove_node_from_referenced_nodes(
        ...
        nodes_to_check.into_iter().for_each(|node_to_check| {
            ...
            let remove_entry = if let Some(ref mut references) =
                self.references_map.get_mut(&node_to_check)
            {
            ...
            let remove_lookup_map = if let Some(ref mut lookup_map) =
                self.referenced_by_map.get_mut(&node_to_check)
            {
    ...
    pub fn delete_node_references(&mut self, source_node: &NodeId) -> bool {
        let deleted_references =
            if let Some(references) = self.references_map.remove(source_node) {
        ...
        let deleted_lookups =
            if let Some(lookup_map) = self.referenced_by_map.remove(source_node) {
        ...
        T: Into<NodeId> + Clone,
    {
        if let Some(ref node_references) = self.references_map.get(source_node) {
            let result =
                self.filter_references_by_type(node_references, &reference_filter);
            if result.is_empty() {
                None
            } else {
                Some(result)
        ...
        match browse_direction {
            BrowseDirection::Forward => {
                if let Some(mut forward_references) =
                    self.find_references(node, reference_filter)
                {
            ...
            BrowseDirection::Invalid => {
                error!(
                    "BrowseDirection::Invalid passed to find_references_by_direction"
                );
                inverse_ref_idx = 0;
            }
        }
