...
macro_rules! server_diagnostics_summary {
    ($address_space: expr, $variable_id: expr, $field: ident) => {
        let server_diagnostics =
            $address_space.server_diagnostics.as_ref().unwrap().clone();
        $address_space.set_variable_getter(
            ...
            move |_, timestamps_to_return, _, _, _, _| {
                ...
                let server_diagnostics_summary =
                    server_diagnostics.server_diagnostics_summary();
                ...
                let mut value =
                    DataValue::from(Variant::from(server_diagnostics_summary.$field));
...
impl AddressSpace {
    ...
    where
        ...
        V: Into<Variant>,
    {
        self.set_variable_value_by_ref(
            &node_id.into(),
            value,
            source_timestamp,
            server_timestamp,
        ...
        match node_class {
            NodeClass::Object => {
                if type_definition.is_null() {
                    ...
                    if let Some(NodeType::ObjectType(_)) =
            ...
            NodeClass::Variable => {
                if type_definition.is_null() {
                    ...
                    if let Some(NodeType::VariableType(_)) =
                        self.find_node(type_definition)
                    {
                        true
                    } else {
    ...
    pub fn is_subtype(&self, subtype_id: &NodeId, base_type_id: &NodeId) -> bool {
        subtype_id == base_type_id || {
            ...
            if let Some(references) = self.find_references(
                base_type_id,
            ...
            if node.node_class() == node_type_class {
                ...
                let nodes = self
                    ...
                    .filter(move |(k, _)| {
                        ...
                        if let Some(type_refs) = self.find_references(
                            k,
    ...
    where
        T: Into<NodeId> + Clone,
    {
        self.references.find_references_by_direction(
            node_id,
            browse_direction,
            reference_filter,
        )
