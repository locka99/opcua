...
use opcua_types::{node_ids::ReferenceTypeId, status_code::StatusCode, *};
...
impl ViewService {
    ...
    pub fn browse(
        ...
        if is_empty_option_vec!(request.nodes_to_browse) {
            ...
            if !view.view_id.is_null() || !view.timestamp.is_null() {
                ...
                if nodes_to_browse.len()
                    <= server_state.operational_limits.max_nodes_per_browse
                {
                    ...
                    let max_references_per_node =
                        if request.requested_max_references_per_node == 0 {
                ...
                <= server_state.operational_limits.max_nodes_per_register_nodes
            {
                if let Some(ref mut callback) = server_state.register_nodes_callback {
                    ...
                    let registered_node_ids =
                        nodes_to_register.iter().map(|n| n.clone()).collect();
    ...
    fn browse_node(
        ...
        if node_to_browse.node_id.is_null()
            || !address_space.node_exists(&node_to_browse.node_id)
        {
        ...
        let node_class_mask =
            NodeClassMask::from_bits_truncate(node_to_browse.node_class_mask);
        ...
        for (idx, reference) in references.iter().enumerate() {
            ...
            if target_node_class != NodeClass::Unspecified && !node_class_mask.is_empty()
            {
                let target_node_class =
                    NodeClassMask::from_bits_truncate(target_node_class as u32);
            ...
            let reference_type_id = if result_mask
            ...
            let is_forward = if result_mask
                .contains(BrowseDescriptionResultMask::RESULT_MASK_IS_FORWARD)
            {
                idx < inverse_ref_idx
            } else {
                true
            ...
            let target_node_class = if result_mask
            ...
            let browse_name = if result_mask
            ...
            let display_name = if result_mask
    ...
    fn browse_from_continuation_point(
        ...
        if let Some(continuation_point) =
            session.find_browse_continuation_point(continuation_point)
        {
            debug!(
                "Browsing from continuation point {}",
                continuation_point.id.as_base64()
            );
            let reference_descriptions =
                continuation_point.reference_descriptions.lock().unwrap();
    ...
    fn reference_description_to_browse_result(
            ...
            && references_remaining > max_references_per_node
        {
            ...
            session.add_browse_continuation_point(BrowseContinuationPoint {
                ...
                reference_descriptions: Arc::new(Mutex::new(
                    reference_descriptions.to_vec(),
                )),
            });
            ...
            let reference_descriptions_slice =
                reference_descriptions[starting_index..].to_vec();
            debug!(
