...
use crate::{
    address_space::AddressSpace, services::Service, session::Session,
    state::ServerState, subscriptions::subscription::Subscription,
};
...
impl SubscriptionService {
    ...
    pub fn async_publish(
        ...
        if session.subscriptions().is_empty() {
            ...
            let result = session.enqueue_publish_request(
                now,
                request_id,
                request.clone(),
                &address_space,
    ...
    pub fn republish(
        ...
        let mut session = trace_write_lock_unwrap!(session);
        let result = session.subscriptions().find_notification_message(
