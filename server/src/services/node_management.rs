...
impl NodeManagementService {
    ...
    fn add_node(
        ...
        let relative_path = RelativePath::from_str(
            &browse_name,
    ...
    fn add_reference(
        ...
        if !session.can_modify_address_space() {
            ...
            if let Some(node_type) =
                address_space.find_node(&item.target_node_id.node_id)
            {
    ...
    fn delete_reference(
        ...
        if !session.can_modify_address_space() {
            ...
            error!(
                "reference cannot be added because only local references are supported"
            );
            StatusCode::BadReferenceLocalOnly
        ...
        } else if target_node_id.is_null() || !address_space.node_exists(&target_node_id)
        {
            ...
            if let Ok(reference_type_id) = item.reference_type_id.as_reference_type_id()
            {
                if item.delete_bidirectional {
                    address_space.delete_reference(
                        &node_id,
                        &target_node_id,
                        reference_type_id,
                    );
