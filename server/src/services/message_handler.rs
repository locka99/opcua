...
impl MessageHandler {
        ...
        let response = match message {
            ...
            SupportedMessage::UnregisterNodesRequest(request) => {
                Self::validate_active_session_service_request(
                    ...
                    move || {
                        self.view_service.unregister_nodes(
                            server_state,
                            session,
                            request,
                        )
            ...
            SupportedMessage::PublishRequest(request) => {
                if let Err(response) = Self::is_authentication_token_valid(
                    session.clone(),
    ...
    fn is_authentication_token_valid(
            ...
            && session.authentication_token() != &request_header.authentication_token
        {
            ...
            Err(
                ServiceFault::new(request_header, StatusCode::BadIdentityTokenRejected)
                    .into(),
            )
        } else {
            Ok(())
    ...
    fn is_session_timed_out(
        ...
        if elapsed.num_milliseconds() as f64 > session.session_timeout() {
            ...
            Err(
                ServiceFault::new(request_header, StatusCode::BadSessionIdInvalid)
                    .into(),
            )
        } else {
            Ok(())
    ...
    fn update_last_service_request_timestamp(
        session: Arc<RwLock<Session>>,
        now: DateTimeUtc,
    ) {
        let mut session = trace_write_lock_unwrap!(session);
    ...
    fn is_session_activated(
        ...
        if !session.is_activated() {
            ...
            Err(
                ServiceFault::new(request_header, StatusCode::BadSessionNotActivated)
                    .into(),
            )
        } else {
            Ok(())
    ...
    fn diag_authorized_request(
        session_diagnostics: &mut SessionDiagnostics,
        authorized: bool,
    ) {
        if authorized {
