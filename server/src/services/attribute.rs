...
impl AttributeService {
    ...
    pub fn write(
        ...
        if is_empty_option_vec!(request.nodes_to_write) {
            ...
            if nodes_to_write.len()
                <= server_state.operational_limits.max_nodes_per_write
            {
    ...
    pub fn history_update(
        ...
        if is_empty_option_vec!(request.history_update_details) {
            ...
            let history_update_details =
                request.history_update_details.as_ref().unwrap();
            let results = history_update_details
                .iter()
                .map(|u| {
                    // Decode the update/delete action
                    let (status_code, operation_results) =
                        Self::do_history_update_details(
                            &decoding_limits,
                            server_state.clone(),
                            address_space.clone(),
                            u,
    ...
    fn decode_history_read_details(
        ...
        let action =
            Self::node_id_to_historical_read_action(&history_read_details.node_id)
                .map_err(|_| StatusCode::BadHistoryOperationInvalid)?;
        match action {
            ObjectId::ReadEventDetails_Encoding_DefaultBinary => {
                Ok(ReadDetails::ReadEventDetails(
                    history_read_details
                        .decode_inner::<ReadEventDetails>(&decoding_limits)?,
                ))
            }
            ObjectId::ReadRawModifiedDetails_Encoding_DefaultBinary => {
            ...
            ObjectId::ReadProcessedDetails_Encoding_DefaultBinary => {
                Ok(ReadDetails::ReadProcessedDetails(
                    history_read_details
                        .decode_inner::<ReadProcessedDetails>(&decoding_limits)?,
                ))
            }
            ObjectId::ReadAtTimeDetails_Encoding_DefaultBinary => {
                Ok(ReadDetails::ReadAtTimeDetails(
                    history_read_details
                        .decode_inner::<ReadAtTimeDetails>(&decoding_limits)?,
                ))
            }
            _ => panic!(),
        }
    ...
    fn decode_history_update_details(
        ...
        let action =
            Self::node_id_to_historical_update_action(&history_update_details.node_id)
                .map_err(|_| StatusCode::BadHistoryOperationInvalid)?;
        match action {
            ObjectId::UpdateDataDetails_Encoding_DefaultBinary => {
                Ok(UpdateDetails::UpdateDataDetails(
                    history_update_details
                        .decode_inner::<UpdateDataDetails>(&decoding_limits)?,
                ))
            }
            ObjectId::UpdateStructureDataDetails_Encoding_DefaultBinary => {
            ...
            ObjectId::UpdateEventDetails_Encoding_DefaultBinary => {
                Ok(UpdateDetails::UpdateEventDetails(
                    history_update_details
                        .decode_inner::<UpdateEventDetails>(&decoding_limits)?,
                ))
            }
            ObjectId::DeleteRawModifiedDetails_Encoding_DefaultBinary => {
            ...
            ObjectId::DeleteAtTimeDetails_Encoding_DefaultBinary => {
                Ok(UpdateDetails::DeleteAtTimeDetails(
                    history_update_details
                        .decode_inner::<DeleteAtTimeDetails>(&decoding_limits)?,
                ))
            }
            ObjectId::DeleteEventDetails_Encoding_DefaultBinary => {
                Ok(UpdateDetails::DeleteEventDetails(
                    history_update_details
                        .decode_inner::<DeleteEventDetails>(&decoding_limits)?,
                ))
            }
            _ => panic!(),
        }
            ...
            Ok(details) => {
                ...
                let result = match details {
                    UpdateDetails::UpdateDataDetails(details) => {
                            ...
                            server_state.historical_data_provider.as_ref()
                        {
                            historical_data_provider
                                .update_data_details(address_space, details)
                        } else {
                    ...
                    UpdateDetails::UpdateEventDetails(details) => {
                            ...
                            server_state.historical_event_provider.as_ref()
                        {
                            historical_event_provider
                                .update_event_details(address_space, details)
                        } else {
                            Err(StatusCode::BadHistoryOperationUnsupported)
                        }
                    }
                    ...
                    UpdateDetails::DeleteAtTimeDetails(details) => {
                        ...
                        {
                            historical_data_provider
                                .delete_at_time_details(address_space, details)
                        } else {
                            Err(StatusCode::BadHistoryOperationUnsupported)
                        }
                    }
                    UpdateDetails::DeleteEventDetails(details) => {
                        ...
                        {
                            historical_event_provider
                                .delete_event_details(address_space, details)
                        } else {
                            Err(StatusCode::BadHistoryOperationUnsupported)
                        }
                    }
    ...
    fn do_history_read_details(
        ...
        let read_details = Self::decode_history_read_details(
            &request.history_read_details,
            &decoding_limits,
        )?;
                        ...
                        if let Some(value) = attribute.value {
                            if let Variant::Byte(value) = value {
                                ...
                                let user_access_level =
                                    UserAccessLevel::from_bits_truncate(value);
                                let user_access_level = session
                                    .effective_user_access_level(
                                        user_access_level,
                                        &node.node_id(),
                                        attribute_id,
                                    );
                        ...
                        if attribute_id == AttributeId::Value {
                            match timestamps_to_return {
                                TimestampsToReturn::Source => {
                                    result_value.source_timestamp =
                                        attribute.source_timestamp;
                                    result_value.source_picoseconds =
                                        attribute.source_picoseconds;
                                }
                                TimestampsToReturn::Server => {
                                    result_value.server_timestamp =
                                        attribute.server_timestamp;
                                    result_value.server_picoseconds =
                                        attribute.server_picoseconds;
                                }
    ...
    fn user_access_level(
        ...
        session.effective_user_access_level(
            user_access_level,
            &node.node_id(),
            attribute_id,
        )
    ...
    fn is_readable(
        ...
        Self::user_access_level(session, node, attribute_id)
        ...
        if let Some(write_mask) = node.as_node().write_mask() {
            match attribute_id {
                ...
                AttributeId::UserWriteMask => {
                    write_mask.contains(WriteMask::USER_WRITE_MASK)
                }
                ...
                AttributeId::ContainsNoLoops => {
                    write_mask.contains(WriteMask::CONTAINS_NO_LOOPS)
                }
                AttributeId::EventNotifier => {
                    write_mask.contains(WriteMask::EVENT_NOTIFIER)
                }
                ...
                AttributeId::ArrayDimensions => {
                    write_mask.contains(WriteMask::ARRAY_DIMENSIONS)
                }
                AttributeId::AccessLevel => write_mask.contains(WriteMask::ACCESS_LEVEL),
                AttributeId::UserAccessLevel => {
                    write_mask.contains(WriteMask::USER_ACCESS_LEVEL)
                }
    ...
    fn write_node_value(
        ...
        if let Some(node) = address_space.find_node(&node_to_write.node_id) {
            if let Ok(attribute_id) = AttributeId::from_u32(node_to_write.attribute_id) {
                let index_range =
                    node_to_write.index_range.as_ref().parse::<NumericRange>();

                if !Self::is_writable(session, &node, attribute_id) {
                    StatusCode::BadNotWritable
                } else if attribute_id != AttributeId::Value
                    && !node_to_write.index_range.is_null()
                {
                ...
                //                    StatusCode::BadWriteNotSupported
                    ...
                    if !data_type_valid {
                        ...
                        let node =
                            address_space.find_node_mut(&node_to_write.node_id).unwrap();
