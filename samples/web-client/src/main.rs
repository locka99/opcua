...
use actix_web::{
    actix::{
        Actor, ActorContext, AsyncContext, Handler, Message, Running, StreamHandler,
    },
    fs, http,
    server::HttpServer,
...
impl OPCUASession {
            ...
            Ok(session) => {
                {
                    ...
                    session.set_connection_status_callback(
                        ConnectionStatusCallback::new(move |connected| {
                            println!(
                    ...
                    session.set_session_closed_callback(SessionClosedCallback::new(
                        |status| {
                ...
                self.session_tx =
                    Some(Session::run_async(self.session.as_ref().unwrap().clone()));
                true
            }
            Err(err) => {
                println!(
    ...
    fn add_event(&mut self, ctx: &mut <Self as Actor>::Context, args: Vec<String>) {
        ...
        if let Some(ref mut session) = self.session {
                ...
                session.create_subscription(2000.0, 100, 300, 0, 0, true, event_callback)
            {
                ...
                let mut item_to_create: MonitoredItemCreateRequest =
                    event_node_id.into();
                item_to_create.item_to_monitor.attribute_id =
                    AttributeId::EventNotifier as u32;
                item_to_create.requested_parameters.filter =
    ...
    fn subscribe(&mut self, ctx: &mut <Self as Actor>::Context, node_ids: Vec<String>) {
        if let Some(ref mut session) = self.session {
            ...
            if let Ok(subscription_id) = session.create_subscription(
                500.0,
                10,
                30,
                0,
