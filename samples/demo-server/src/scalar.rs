...
pub fn scalar_random_value(id: DataTypeId) -> Variant {
    ...
    match id {
        ...
        DataTypeId::DateTime => {
            DateTime::from(rng.gen_range::<i64, i64, i64>(0, DateTime::endtimes_ticks()))
                .into()
        }
...
fn add_dynamic_scalar_variables(
    server: &mut Server,
    ns: u16,
    dynamic_folder_id: &NodeId,
) {
    // The address space is guarded so obtain a lock to change it
    let address_space = server.address_space();
    let mut address_space = address_space.write().unwrap();
...
fn add_dynamic_array_variables(
    server: &mut Server,
    ns: u16,
    dynamic_folder_id: &NodeId,
) {
    ...
    server.add_polling_action(250, move || {
        ...
        SCALAR_TYPES.iter().for_each(|sn| {
            ...
            let values = (0..10)
                ...
                .collect::<Vec<Variant>>();
            let _ =
                address_space.set_variable_value_by_ref(&node_id, values, &now, &now);
        });
    });
}
