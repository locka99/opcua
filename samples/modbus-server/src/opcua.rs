...
impl AliasGetterSetter {
    ...
    fn get_alias_value(
        ...
        let value = match table {
            Table::OutputCoils => Self::value_from_coil(
                address,
                &runtime.config.output_coils,
                &runtime.output_coils,
            ),
            Table::InputCoils => Self::value_from_coil(
                address,
                &runtime.config.input_coils,
                &runtime.input_coils,
            ),
    ...
    fn set_alias_value(
        ...
        match table {
            ...
            Table::OutputRegisters => {
                ...
                // Write the words
                let (_, words) = Self::value_to_words(value)
...
fn values_1_word() {
    ...
    // SByte
    assert_eq!(
        AliasGetterSetter::word_to_value(
            AliasType::SByte,
            u16::from_le_bytes([0x81, 0xff])
        ),
        Variant::SByte(-127i8)
    );
    assert_eq!(
...
fn values_4_words() {
    // UInt64
    assert_eq!(
        AliasGetterSetter::words_to_value(
            AliasType::UInt64,
    ...
    if let Variant::Double(v) = AliasGetterSetter::words_to_value(
        AliasType::Double,
        &[0x4093, 0x4A45, 0x6D5C, 0xFAAD],
    ) {
        // Expect value to be 1234.5678
